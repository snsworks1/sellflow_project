...
Tailwind CSS의 darkMode: 'class' 설정 적용

localStorage 기반 다크모드 상태 유지

레이아웃 전반(class, bg, text 등)에 dark: 클래스 적용

헤더 우측 버튼으로 라이트/다크모드 전환 가능

상품 수집 페이지 스타일 개선

테이블 구조 및 색상 다크모드 대응 (dark:bg-gray-700, dark:text-gray-200 등)

hover 시 다크모드에서도 가독성 좋은 배경색 적용

테이블 전체 항목을 text-center로 정렬 (상품명 제외)

체크된 행 강조(selected-row) 및 hover 시 색상 분리 적용

폼 섹션 자동 접힘 기능: 수집된 상품이 있을 경우 폼은 접힌 상태로 시작됨

드롭다운/셀렉트 박스/버튼 영역도 다크모드 대응 완료

레이아웃 구성 보완

table과 thead, tbody 스타일 세분화

수직 정렬(align-middle) 및 이미지 mx-auto로 정확한 가운데 정렬
